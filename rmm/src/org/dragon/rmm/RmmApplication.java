...
public class RmmApplication extends Application {
	...
	public void onCreate() {
		...
		StringResource.setContext(getApplicationContext());
	...
	/**
	 * 注册地理位置监听器，开启监听。监听器最好是在onResume方法中注册，在onPause中调用removeLocationUpdates注销。
	 * 注册成功后， 就可以使用getLastKnownLocation获取Location地理位置。
	 * 
	 * @param gps
	...
	/**
	 * 注册地理位置监听器，开启监听，并且传入观察者，可以让观察者异步操作UI变化，做到实时变化。监听器最好是在onResume方法中注册，
	 * 在onPause中调用removeLocationUpdates注销。注册成功后，
	...
	/**
	 * 第一次启动时，因为没SharedPreferences文件，所以为初始化值，比如true要显示，然后在将这个值赋为false，保存后，
	 * 下次启动是读取SharedPreferences文件，找到值就为false。 你在后面写判断要不要显示引导界面就好了。
	 * 
	 * @return 是否是第一次启动应用
	 */
