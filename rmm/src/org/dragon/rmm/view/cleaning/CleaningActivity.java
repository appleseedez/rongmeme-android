...
public class CleaningActivity extends Activity {
	...
	private void initStarComponents(List<CleaningItemVO> list) {
		...
		for (CleaningItemVO cbi : list) {
			...
			if (starlevel == 1) {
				bigCategoryStartLevelImageview
						.setImageResource(R.drawable.onestart);
				bigCategoryPullDownImageButton.performClick();

			} else if (starlevel == 2) {
				bigCategoryStartLevelImageview
						.setImageResource(R.drawable.twostart);
			} else if (starlevel == 3) {
				bigCategoryStartLevelImageview
						.setImageResource(R.drawable.threestart);
			} else if (starlevel == 4) {
				bigCategoryStartLevelImageview
						.setImageResource(R.drawable.fourstart);
			} else {
				bigCategoryStartLevelImageview
						.setImageResource(R.drawable.fivestart);
			}
	...
	OnClickListener bigCategoryPullDownOnClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			View starSmallContent = starItem
					.findViewById(R.id.si_small_category_content);
			RelativeLayout bigStarItem = (RelativeLayout) starItem
					.findViewById(R.id.si_top_navigation_bar);
			if (starSmallContent.getVisibility() == View.GONE) {
				bigStarItem.setBackgroundColor(getResources().getColor(
						R.color.white));
	...
	OnClickListener startItemViewOnClickListener = new OnClickListener() {
		...
		@Override
		public void onClick(View v) {
			CleaningItemVO cleaningBigItem = (CleaningItemVO) v
