...
public class HourlyEmployeeListViewAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            // 获取组件布局
            convertView = layoutInflater.inflate(R.layout.hourlyemployee_xlistview_item, null);
            mailListViewHolder.employeeHeadPortraitImageView = (ImageView) convertView
                    .findViewById(R.id.employeeHeadPortraitImageView);
            mailListViewHolder.employeeNameTextView = (TextView) convertView.findViewById(R.id.employeeNameTextView);
            mailListViewHolder.priceRangeTextView = (TextView) convertView.findViewById(R.id.priceRangeTextView);
            mailListViewHolder.skillsTextView = (TextView) convertView.findViewById(R.id.skillsTextView);
            mailListViewHolder.employeeCheckBox = (CheckBox) convertView.findViewById(R.id.employeeCheckBox);

            final int p = position;
            mailListViewHolder.employeeCheckBox.setOnClickListener(new View.OnClickListener() {
            ...
            mailListViewHolder = (HourlyEmployeeViewHolder) convertView.getTag();
        }
        // 绑定数据、以及事件触发
    ...
    private void bingUiAndData(HourlyEmployeeViewHolder mailListViewHolder, int position) {
        ...
        if (!TextUtils.isEmpty(headPortraitUrlPath)) {
            mImageFetcher.loadImage(headPortraitUrlPath, mailListViewHolder.employeeHeadPortraitImageView);
        } else {
            mailListViewHolder.employeeHeadPortraitImageView.setImageResource(R.drawable.default_head_portrait);
        }

        // 获取名字
        mailListViewHolder.employeeNameTextView.setText(data.get(position).getName());
        // 获取价格范围字符串
        StringBuilder priceRangeSb = new StringBuilder(20).append("￥   ")
                .append(Double.valueOf(data.get(position).getLowprice()).toString()).append("  -  ")
                .append(Double.valueOf(data.get(position).getPrice()).toString()).append("  /  2h");
        mailListViewHolder.priceRangeTextView.setText(priceRangeSb.toString());
        // 设置技巧文本
        mailListViewHolder.skillsTextView.setText(data.get(position).getSkills());
